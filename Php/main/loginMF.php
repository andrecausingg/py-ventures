            ...
            if($check_email_row > 0){
                ...
                while($row = mysqli_fetch_assoc($check_email)){
                    ...
                    if($uat_password == $this->password){
                        $check_uat_id = mysqli_query($classDataBaseConnection->connect(), "SELECT * FROM user_map_level_progress_tbl WHERE umlpt_uat_id ='$uat_id'");
                        $check_uat_id_row = mysqli_num_rows($check_uat_id); // Get the data on specific row database
